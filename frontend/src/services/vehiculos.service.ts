...
export const vehiculosService = {
  ...
  crear: (data: { placa: string; id_usuario: number; id_tipo: number; id_marca?: number; marca_personalizada?: string; color?: string; modelo?: string }) =>
    api.post<{ mensaje?: string; data?: unknown }>('/vehiculos', data),
  actualizar: (placa: string, data: Partial<{ id_usuario: number; id_tipo: number; id_marca: number; marca_personalizada: string; color: string; modelo: string; activo: boolean }>) =>
